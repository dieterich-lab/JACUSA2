...
A user can provide base substitution(s) to partition reads into two sets 
% (TODO4CD: what is a typical experiment for this?) 
...
We developed and tested JACUSA2  with Java 17. 
If you encounter Java-related problems, consider switching to Java 17.
% --------------------------------------------------------------------------------------------------
...
We have downloaded primary sequencing data from \cite{Zhou2018}.
% and mapped raw reads according to
...
Check the manual of SAMtools/BCFtools\footnote{Check: \url{https://www.htslib.org/}} or
...
\begin{description}
  ...
  \item[sort BAM] \begin{verbatim} samtools sort -o mapping.sorted.bam mapping.bam \end{verbatim} 
...
\begin{description} 
  ...
  \item[second strand sequenced] ``-P1 UNSTRANDED -P2 FR-SECONDSTRAND''
\end{description}
...
\begin{verbatim}
java -jar JACUSA2.jar call-2 -r JACUSA2.out -s cDNA_1.bam cDNA_2.bam
\end{verbatim}
...
\begin{verbatim}
java -jar JACUSA2.jar call-2 -a E:file=snps.vcf:type=VCF -r JACUSA2.out -s \
...
in BAM files or by providing a <reference.fasta> via command line: ``-R reference.fasta''.
...
\input{CLI/q2}
%---------------------------------------------------------------------------------------------------
\subsubsection{Enforce/override base call quality}
\input{CLI/Q}
%---------------------------------------------------------------------------------------------------
...
{\small
  \begin{tabular}{ll}
    ...
    \hline
    htsjdk 4.1.3              & \url{https://github.com/samtools/htsjdk} \\
    Apache:                   & \\
