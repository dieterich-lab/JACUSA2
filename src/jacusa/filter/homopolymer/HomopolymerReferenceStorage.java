 ...
 * multiple instances each condition and replicate have its own instance. The
 * reference sequence is for all identical. Compute homopolymer once for one
 * instance and reuse for others. Needs optimization.
 * 
...
public class HomopolymerReferenceStorage extends AbstractStorage {
	...
	public HomopolymerReferenceStorage(final SharedStorage sharedStorage, final DataType<BooleanData> dataType,
			final int minLength, final int bamFiles) {
	...
	HomopolymerStorage add(final Coordinate coordinate) {
		final HomopolymerStorage storage = new HomopolymerStorage(getSharedStorage(), dataType, minLength);
