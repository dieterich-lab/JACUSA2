...
import jacusa.worker.LinkageRTArrestWorker;
...
public class LinkageRTArrestFactory<T extends AbstractData & hasCoverage & hasReferenceBase & hasReadInfoExtendedCount> 
extends AbstractMethodFactory<T, StatisticResult<T>> {
	...
	public Map<Character, AbstractResultFormat<T, StatisticResult<T>>> getResultFormats() {
		...
		resultFormat = new BED6lrtArrestResultFormat2<T, StatisticResult<T>>(getParameter());
	...
	@Override
	public LinkageRTArrestWorker<T> createWorker(final int threadId) {
		return new LinkageRTArrestWorker<T>(getWorkerDispatcher(), threadId,
