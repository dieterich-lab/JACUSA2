...
abstract class INDELCountResult implements Result {
	...
	INDELCountResult(final SortedSet<BaseSub> baseSubs, final Result result, final MinkaParameter minkaParameter,
	...
	private String getKey(final int condition, final int replicate) {
		return new StringBuilder().append(getField()).append(condition + 1).append(replicate + 1).toString();
	}

	private String getValue(final int count, final int coverage) {
		return new StringBuilder().append(count).append(InputOutput.VALUE_SEP).append(coverage).toString();
	}
	...
	private boolean addTotalCount(final int valueIndex, final int condition, final int replicate) {
		...
		final int coverage = container.get(DataType.get("reads", IntegerData.class)).getValue();
	...
	private boolean addStratifiedCount(final int valueIndex, final int condition, final int replicate) {
		...
		final int coverage = container.get(DataType.get("reads", BaseSub2Integer.class)).getMap().get(baseSub).getValue();
		addCount(valueIndex, condition, replicate, count, coverage);
		return count > 0;
	}
	...
	private void addCount(final int valueIndex, final int condition, final int replicate, final int count,
			final int coverage) {
