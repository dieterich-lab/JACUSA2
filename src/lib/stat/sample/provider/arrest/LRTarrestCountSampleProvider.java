...
public class LRTarrestCountSampleProvider implements EstimationSampleProvider {
	...
	public NominalData createData(final List<DataContainer> dataContainers) {
		...
		for (int replicateIndex = 0; replicateIndex < dataContainers.size(); replicateIndex++) {
			...
			final int readArrestCount 	= dataContainer.getArrestPos2BaseCallCount()
					.getArrestBaseCallCount(onePosition).getCoverage();
			final int readThroughCount 	= dataContainer.getArrestPos2BaseCallCount()
					.getTotalBaseCallCount().getCoverage() - readArrestCount;
